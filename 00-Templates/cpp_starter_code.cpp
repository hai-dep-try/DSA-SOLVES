 ...
 */
const std::string VERSION = "0.4.<25024149>";
...
bool selectBothBotLevels(BotLevel* levels);
...
void clearScreen();
void showSelectMenu(SelectType selectType, int currentSize = 0);
...
pII findImmediateMove(char board[][BOARD_N_MAX], const int size, const int goal, const char symbol);
int evaluateDirection(char board[][BOARD_N_MAX], const int size, const pII move, const char symbol, const int dx, const int dy);
int evaluateMove(char board[][BOARD_N_MAX], const int size, const int goal, const pII move, const char botSymbol, const char playerSymbol);
pII bestScoredMove(char board[][BOARD_N_MAX], const int size, const int goal, const char botSymbol, const char playerSymbol);
pII imperfectHeuristicMove(char board[][BOARD_N_MAX], const int size, const int goal, const char botSymbol, const char playerSymbol, const int randomRatePercent);
bool readCoordinateMove(int* row, int* col);
 ...
 */
bool validateInput(std::string& input) {
    // Remove leading/trailing whitespace, including CR/LF from redirected input.
    ...
    if (input.empty()) {
        GameLogger::log("validateInput error", GameLogger::Level::ERROR);
        return false; 
    }
...
bool getInput(int* val) {
    ...
    if (!std::getline(std::cin >> std::ws, input)) {
        GameLogger::log("Reached End of File (EOF) or Stream Error. Exiting.", GameLogger::Level::ERROR);
        exit(1);
    }
    if(!validateInput(input)){
        GameLogger::log("Invalid Input", GameLogger::Level::ERROR);
        return false;
    }
    else{
        try {
        ...
        GameLogger::log("Success Input", GameLogger::Level::INFO);
        return true; 
    }
}
...
bool selectSize(int* size) {
    ...
    if (getInput(size)){
        if(*size >=3 && *size <= BOARD_N_MAX){
            ...
            GameLogger::log("Invalid input. Board size must be between 3 and " + std::to_string(BOARD_N_MAX) + ". Please try again.");
            return false;
        }
    }else {
        std::cin.clear();
        GameLogger::log("Invalid input. Please enter an interge!");
        return false;
    }
}
...
bool selectGoal(int* goal, const int size) {
    ...
    if (getInput(goal)){
        if (*goal >= 3 && *goal <= size){
            ...
            GameLogger::log("Invalid input. Board size must be between 3 and" + std::to_string(size)+". Please try again.");
            return false; 
        }
    }else{
        GameLogger::log("Invalid input. Please enter an interger!");
        return false;
    }
}
/**
...
bool selectGameMode(GameMode* mode) {
    ...
    if (getInput(&choice)){
        if (choice>= 1 && choice <= 3){
            ...
            GameLogger::log("Lỗi. Lựa chọn phải là số từ 1 đến 4. Vui lòng thử lại!");
            return false; 
        }
    }else{
        GameLogger::log("Lỗi. Đầu vào không hợp lệ. Vui lòng chỉ nhập số nguyên!");
        return false;
    }
}
...
bool selectBotLevel(BotLevel* levels, const int index) {
    ...
    if (getInput(&choice)){
        if (choice >= 1 && choice <= 3){
            ...
            GameLogger::log("Lỗi. Lựa chọn phải là 1, 2 hoặc 3. Vui lòng nhập lại");
            return false;
        }
    }else{
        GameLogger::log("Lỗi. Đầu vào không hợp lệ. Vui lòng chỉ nhập số nguyên!"); 
...
/**
 * Ask the user to select difficulty for both bots in one line.
 *
 * Expected input:
 *   level1 level2
 *
 ...
 *
 * Parameters:
 *   levels : BotLevel array with at least 2 elements
 *
 * Return:
 *   true  -> both levels selected successfully
 *   false -> invalid input
 */
bool selectBothBotLevels(BotLevel* levels) {
    std::string input;
    if (!std::getline(std::cin >> std::ws, input)) {
        GameLogger::log("Invalid input. Please enter two integers!", GameLogger::Level::ERROR);
        return false;
    }
    ...
    if (!(ss >> first >> second) || (ss >> extra)) {
        GameLogger::log("Invalid input. Please enter two integers like: 1 2", GameLogger::Level::ERROR);
        return false;
    }

    auto toLevel = [](int choice, BotLevel& out) -> bool {
        switch (choice) {
            case 1:
                out = BotLevel::EASY;
                return true;
    ...
    if (!toLevel(first, levels[0]) || !toLevel(second, levels[1])) {
        GameLogger::log("Invalid input. Each bot level must be 1, 2, or 3.", GameLogger::Level::ERROR);
        return false;
    }
...
bool getPlayerMove(int* row, int* col) {
    if (!getInput(row)){
        GameLogger::log("Invalid input. Dữ liệu không hợp lệ. Vui lòng chỉ nhập số nguyên.");
        return false;
    }
    if (!getInput(col)){
        GameLogger::log("Invalid input. Vui lòng chỉ nhập số nguyên.");
        return false;
    }
    return true;
}
 ...
 */
void showSelectMenu(SelectType selectType, int currentSize) {
    switch (selectType) {
        ...
        case SelectType::SIZE_UI:
            ...
            std:: cout << "=============== BOARD SIZE ===============\n";
            std:: cout << "Enter board size (From 3 to " << BOARD_N_MAX << ") :";
            break;
            std:: cout << "\n"; 
        ...
        case SelectType::GOAL_UI:
            ...
            std:: cout << "============== Winning Condition =============\n"; 
            std:: cout << "Enter number of marks in a row to win (From 3 to " << currentSize << "):";
            break;
            std:: cout << "\n"; 
        ...
        case SelectType::GAME_MODE_UI:
            ...
            std:: cout << "2. Player vs Bot (PvE)\n";
            std:: cout << "3. Bot vs Bot (EvE)\n";
        ...
        case SelectType::PLAYER_UI:
            ...
            std:: cout << "Enter move coordinates (Row Column, e.g: 0 1):";
            break;
            std:: cout << "\n"; 
        ...
        case SelectType::MUL_BOT_LEVEL_UI:
            ...
            std:: cout << "============= SELECT DIFFICULTY FOR BOTH BOTS =============\n";
            std:: cout << "Enter difficulty for Bot 1 and Bot 2 (e.g., 1 2): \n";
            std:: cout << "1. EASY \n";
            std:: cout << "2. MEDIUM \n";
            std:: cout << "3. HARD \n";
            break;
...
void displayBoard(const char board[][BOARD_N_MAX], const int size) {
    ...
    for (int col = 0; col < size; col++){
        if (col< 10){
            std::cout << " " << col << "  "; //2 khoảng trắng sau số có 1 chữ số
        }else{
            std:: cout << " " << col << " "; // 1 khoảng trắng sau số có 2 chữ số
        }
    }
    std:: cout << "\n";
    std:: cout << "   "; 
...
void showPlayer(int player, bool is_bot) {
    // TODO:
    if(is_bot){
        std::cout << "Bot (Player " << player << " ) is thinking..." << std:: endl;
    } else{ 
...
void showResult(const int winner, const bool is_bot) {
    ...
    if (winner == 0){
        ...
        if (is_bot){
            std:: cout << "Bot (Player " << winner << ") wins!" << std::endl; 
        }else{
...
void startGame(const RunConfig& config,
    ...
    int goal = 0;
    do{
        showSelectMenu(SelectType::GOAL_UI, gameSetup.size); 
    }while (!selectGoal(&goal, gameSetup.size));
    gameSetup.goal = goal;
    ...
    if (gameSetup.mode == GameMode::PVE){
        BotLevel level = BotLevel::INVALID_LV;
        do{
            showSelectMenu(SelectType::BOT_LEVEL_UI); 
        }while (!selectBotLevel(&level, 1));
        gameSetup.levels[1] = level;
    }
    // 7. If mode == EVE
    ...
    if (gameSetup.mode == GameMode::EVE){
        ...
        } while (!selectBothBotLevels(gameSetup.levels));
    }
    // 8. Initialize board
...
GameResult playGame(const RunConfig& config,
    ...
    if(isBotTurn){
        ...
        readCoordinateMove(&row, &col);
    }
    //      d) validate move
    //         isValidMove(...)
    if (!isValidMove(gameSetup.board, gameSetup.size, row, col)){
        if (!isBotTurn && config.interactive){
    ...
    //      g) check draw
    //         checkDraw(...)
    if (turns == gameSetup.size * gameSetup.size) {
            result.winner = 0; 
            result.turns = turns;
            result.isBot = false;
            break;
            ...
            for(int d = 0; d < 4; d++){
                ...
                if (count == goal){
                    if (rule == EndRule::OPEN_TWO){
                        bool head1 = isEmptyHead(board, size, i - dx[d], j - dy[d], symbol);
                        bool head2 = isEmptyHead(board, size, r, c, symbol);
...
pII findImmediateMove(char board[][BOARD_N_MAX],
                      const int size,
                      const int goal,
                      const char symbol) {
    for (int row = 0; row < size; row++) {
        for (int col = 0; col < size; col++) {
            ...
            board[row][col] = symbol;
            bool winsNow = checkWin(board, size, symbol, goal, EndRule::NONE);
            board[row][col] = '-';
...
int evaluateDirection(char board[][BOARD_N_MAX],
                      const int size,
                      const pII move,
                      const char symbol,
                      const int dx,
    ...
    int row = move.first + dx;
    int col = move.second + dy;
...
int evaluateMove(char board[][BOARD_N_MAX],
                 const int size,
                 const int goal,
                 const pII move,
    ...
    board[move.first][move.second] = botSymbol;
    for (int dir = 0; dir < 4; dir++) {
        score += evaluateDirection(board, size, move, botSymbol, dx[dir], dy[dir]);
    }
    ...
    board[move.first][move.second] = playerSymbol;
    for (int dir = 0; dir < 4; dir++) {
        score += evaluateDirection(board, size, move, playerSymbol, dx[dir], dy[dir]) * 2;
    }
    ...
    int center = size / 2;
    int distance = std::abs(move.first - center) + std::abs(move.second - center);
    score += (size * 2 - distance);

    return score;
}

pII bestScoredMove(char board[][BOARD_N_MAX],
                   const int size,
                   const int goal,
                   const char botSymbol,
                   const char playerSymbol) {
    ...
    for (int row = 0; row < size; row++) {
        for (int col = 0; col < size; col++) {
            ...
            pII move = {row, col};
            int score = evaluateMove(board, size, goal, move, botSymbol, playerSymbol);
            if (score > bestScore) {
                bestScore = score;
                bestMove = move;
            }
...
bool readCoordinateMove(int* row, int* col) {
    std::string input;
    if (!std::getline(std::cin >> std::ws, input)) {
        GameLogger::log("Invalid input. Please enter two integers like: 0 0", GameLogger::Level::ERROR);
        return false;
    }
...
pII imperfectHeuristicMove(char board[][BOARD_N_MAX],
                           const int size,
                           const int goal,
                           const char botSymbol,
                           const char playerSymbol,
                           const int randomRatePercent) {
    pII winningMove = findImmediateMove(board, size, goal, botSymbol);
    if (winningMove.first != -1) {
        return winningMove;
    }

    pII blockingMove = findImmediateMove(board, size, goal, playerSymbol);
    if (blockingMove.first != -1) {
        return blockingMove;
    }

    int errorRoll = (rand() % 100) + 1;
    if (errorRoll <= randomRatePercent) {
        return random_pick(board, size);
    }

    return bestScoredMove(board, size, goal, botSymbol, playerSymbol);
}
...
pII simple_heuristic(char board[][BOARD_N_MAX],
                     ...
                     const char botSymbol,
                     const char playerSymbol) {
    return imperfectHeuristicMove(board, size, goal, botSymbol, playerSymbol, 18);
}
...
pII hard_level(char board[][BOARD_N_MAX],
               ...
               const char botSymbol,
               const char playerSymbol) {
    return imperfectHeuristicMove(board, size, goal, botSymbol, playerSymbol, 2);
}
